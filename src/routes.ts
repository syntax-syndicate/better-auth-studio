...
export function createRoutes(
  ...
  router.post('/api/tools/plugin-generator', async (req: Request, res: Response) => {
    try {
      ...
      const generateHookCode = (hook: any) => {
      ...
      const generateMiddlewareCode = (mw: any) => {
...
${hooks.length > 0 ? '\n  // Hooks configuration\n' : ''}${hooks
  .map(
    (hook: any) =>
      `  ${hook.name}?: (context: any) => Promise<void> | void;`
  )
  .join('\n')}
${middleware.length > 0 ? '\n  // Middleware configuration\n' : ''}${middleware
  .map(
    (mw: any) =>
      `  ${mw.name}?: (req: any, res: any, next: any) => Promise<void> | void;`
  )
  .join('\n')}
${rateLimit ? '\n  // Rate limiting\n  rateLimit?: {\n    windowMs?: number;\n    max?: number;\n  };' : ''}
}
...
${table.fields?.map((field: any) => `            ${field.name}: {
              type: "${field.type}",
