...
export function createRoutes(authConfig: AuthConfig) {
  ...
  router.get('/api/users/:userId', async (req: Request, res: Response) => {
    try {
      ...
      const users = await adapter.findMany({ model: 'user', limit: 10000 });
      const user = users && users.length > 0 ? users.find((u: any) => u.id === userId) : null;
      if (!user) {
  ...
  // Combined organization endpoint that handles both :id and :orgId patterns
  router.get('/api/organizations/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (!adapter) {
        console.log('No auth adapter available');
        res.status(500).json({ success: false, error: 'Auth adapter not available' });
        return;
      }

      if (typeof adapter.findMany !== 'function') {
        console.log('findMany function not available on adapter');
        res.status(500).json({ success: false, error: 'Database query not supported' });
        return;
      }

      // Try to find organization by ID using where clause if supported
      let organization = null;
      
      try {
        // First try with where clause for efficient querying
        console.log('Trying to find organization with where clause');
        ...
        console.log(`Found organization with where clause: ${organization ? 'Yes' : 'No'}`);
      } catch (whereError) {
        console.log('Where clause not supported, falling back to full scan');
        // Fallback to fetching all and filtering (less efficient but works)
        try {
          const allOrganizations = await adapter.findMany({ model: 'organization' });
          console.log(`Total organizations found: ${allOrganizations?.length || 0}`);
          if (allOrganizations?.length > 0) {
            console.log('Organization IDs:', allOrganizations.map((org: any) => org.id));
          }
          organization = allOrganizations?.find((org: any) => org.id === id);
          console.log(`Found organization after filtering: ${organization ? 'Yes' : 'No'}`);
        } catch (fallbackError) {
      ...
      // If no adapter or organization not found, return 404
