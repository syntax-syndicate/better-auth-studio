...
import { startStudio } from 'better-auth-studio';
import { findAuthConfig } from 'better-auth-studio';
  ...
  .action(async (options) => {
    try {
      ...
      if (authConfig.socialProviders && typeof authConfig.socialProviders === 'object') {
        ...
        const providerNames = authConfig.providers.map((p: any) => p.type || p.name).filter(Boolean);
        if (providerNames.length > 0) {
          providersInfo = providerNames.join(', ');
        }
      }
